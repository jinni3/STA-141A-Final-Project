...
# Introduction 
In this project, we are working with a total of 18 RDS files which contain results from 18 sessions of experiments. The experiments consist of a single mouse receiving stimuli (left and right contrast) to test an outcome (feedback type). The neural activity (spks), time of activity (time), and the area of the brain where the neuron lives (brain_area) are also all recorded. The primary objective of the project is to build a predictive model to predict to outcome of each trial using the neural activity and stimuli data. 
